...
const removeImageFromPreloaded = (index) => {
    form.value.preloaded_images.splice(index, 1)
}
...
const submit = () => {
    ...
    axios
        .post(`/admin/products/${props.product.id}`, form.value, {headers : {'content-type': 'multipart/form-data'}})
        .then((response) => {
            errors.value = {}
...
onMounted(() => {
    ...
    form.value.preloaded_images = props.product.images
});
                    ...
                    <div class="col-12">
                        ...
                        <div class="row">
                            ...
                            <div class="col-12 order-2 order-sm-3 mb-5 mt-3 mb-sm-0">
                                ...
                                <InputError class="mt-2" :message="errors.preloaded_images" />

                                <InputError class="mt-2" :message="errors.images" />

                                <InputError class="mt-2" :message="errors.total_images" />

                                <InputError class="mt-2" :message="errors.image_items" />

                                <div id="image-upload-grid" class="image-upload-grid">

                                    <div v-for="(image, index) in form.preloaded_images" :key="index" class="image-upload-item mx-auto">
                                        <div class="card text-center rounded-0 x-shadow-hover-bold">
                                            <div class="card-body p-0 position-relative">
                                                <img :src="image.path" class="img-fluid image-gallery" alt="Image"
                                                     data-toggle="tooltip" data-placement="bottom">
                                                <span class="btn-remove" tabindex="0" data-toggle="tooltip"
                                                      data-placement="left" title="Eliminar" @click="removeImageFromPreloaded(index)"></span>
                                            </div>
                                        </div>
                                    </div>
